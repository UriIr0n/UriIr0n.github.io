...
(() => {
  ...
  function renderProjects() {
    ...
    if (countEl) countEl.textContent = capFilter
      ...
      : `${PROJECTS.length} דשבורדים · הסלייסר עובד באמת`;
  }
  renderProjects();

  /* ---------- חותמת רענון בעמוד הבית (כמו בדשבורד אמיתי) ---------- */
  const refreshEl = document.getElementById('hero-refresh');
  if (refreshEl) refreshEl.textContent = '· רענון אחרון: ' +
  ...
  (function ghostDash() {
    ...
    for (let i = 0; i < 26; i++) {
      ...
      const bar = el('rect', {
        ...
        fill: barColors[i % 3], opacity: 0.13, class: 'gd-bar',
      });
